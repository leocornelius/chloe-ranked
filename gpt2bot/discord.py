...
async def on_message(message):
    ...
    if not (message.author == client.user): #Check to ensure the bot does not respond to its own messages
        if(message.mention_everyone == False):
            if(client.user.mentioned_in(message) or isinstance(message.channel, discord.abc.PrivateChannel) or needMention == False): #Check if the bot is mentioned or if the message is in DMs
                async with message.channel.typing(): #Show that the bot is typing
                    ...
                    print(message.author + ": " + txtinput + ", " + txt)
                    #gifchance = 18
